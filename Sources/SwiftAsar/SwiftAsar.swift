...
public struct AsarArchive: Sendable {
    ...
    public func readHeader() async throws -> ArchiveHeader {
        // Check cache first
    ...
    public func extractFile(filename: String, followLinks: Bool = true) async throws -> Data {
        ...
        guard case .file(let fileEntry) = entry else {
        ...
        if fileEntry.unpacked {
            ...
            try await verifyIntegrity(data: fileData!, integrity: fileEntry.integrity)
